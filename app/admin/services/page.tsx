...
const ServicesCMS: React.FC<ServicesCMSProps> = () => {
  ...
  const [currentData, setCurrentData] = useState<ServicesConfig>(DEFAULT_SERVICES_CONFIG);
  const [sectionData, setSectionData] = useState<SectionFormData>({
    ...
    enableProgressAnimation: DEFAULT_SERVICES_CONFIG.enableProgressAnimation || true,
  });
  ...
  const loadServicesData = async () => {
    try {
      ...
      setServices(data.features.map((service: ServiceFeature, index: number) => ({
        ...service,
        id: `service-${index}`,
      })));
    } catch (error) {
  ...
  const resetToDefault = async () => {
    if (confirm("Are you sure you want to reset to default? This will delete all custom changes.")) {
      try {
        setIsSaving(true);
  ...
  const updateBulletPoint = (serviceId: string, index: number, value: string) => {
    const service = services.find(s => s.id === serviceId);
    if (service) {
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            onClick={saveServicesData}
                              leftIcon={isSaving ? <Loader /> : <LucideIcons.Save className="w-4 h-4" />}
            disabled={isSaving}
        ...
        {saveStatus !== "idle" && (
          ...
          >
            <div className="flex items-center gap-2">
              {saveStatus === "success" && <LucideIcons.CheckCircle className="w-4 h-4" />}
              {saveStatus === "error" && <LucideIcons.XCircle className="w-4 h-4" />}
            ...
            <TabsContent value="section" className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => setSectionData({ ...sectionData, highlight: e.target.value })}
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setSectionData({ ...sectionData, slidesToShow: parseInt(value) })}
                      >
                        {[1, 2, 3, 4, 5, 6].map(num => (
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-medium">Slides to Scroll</label>
                      <Select
                        value={sectionData.slidesToScroll.toString()}
                        onValueChange={(value) => setSectionData({ ...sectionData, slidesToScroll: parseInt(value) })}
                      >
                        {[1, 2, 3].map(num => (
                          <option key={num} value={num}>{num}</option>
                        ))}
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium">Autoplay Speed (ms)</label>
                    <Input
                      type="number"
                      value={sectionData.autoplaySpeed}
                      onChange={(e) => setSectionData({ ...sectionData, autoplaySpeed: parseInt(e.target.value) })}
                      min={1000}
                      max={10000}
                      step={500}
                    />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setSectionData({ ...sectionData, showNavigation: !!checked })}
                      />
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setSectionData({ ...sectionData, showAutoplay: !!checked })}
                      />
                      <label htmlFor="showAutoplay" className="text-sm font-medium">
                        Enable Autoplay
                      </label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setSectionData({ ...sectionData, showDots: !!checked })}
                      />
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setSectionData({ ...sectionData, showArrows: !!checked })}
                      />
                      <label htmlFor="showArrows" className="text-sm font-medium">
                        Show Arrows
                      </label>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => setSectionData({ ...sectionData, enableHoverEffects: !!checked })}
                        />
                      ...
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => setSectionData({ ...sectionData, enableFlipAnimation: !!checked })}
                        />
                        <label htmlFor="enableFlipAnimation" className="text-sm">
                          Flip Animation
                        </label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => setSectionData({ ...sectionData, enableProgressAnimation: !!checked })}
                        />
                        <label htmlFor="enableProgressAnimation" className="text-sm">
                          Progress Animation
                        </label>
                      </div>
                    </div>
              ...
              <div className="space-y-4">
                {services.map((service, index) => (
                  <ServiceCard
                    ...
                    onMoveDown={() => index < services.length - 1 && moveService(index, index + 1)}
                    onAddBulletPoint={() => addBulletPoint(service.id)}
                    onUpdateBulletPoint={(bulletIndex, value) => updateBulletPoint(service.id, bulletIndex, value)}
                    onRemoveBulletPoint={(bulletIndex) => removeBulletPoint(service.id, bulletIndex)}
                ...
                <div className="space-y-4">
                  <div className="text-center p-4 bg-muted rounded-lg">
                    <h3 className="font-semibold text-lg">
                      {sectionData.title}{" "}
                      <span className="text-primary">{sectionData.highlight}</span>
                    </h3>
                  ...
                  <div className="space-y-2">
                    <h4 className="font-medium">Active Services: {services.filter(s => s.isActive).length}</h4>
                    <div className="space-y-1 max-h-60 overflow-y-auto">
                      {services
                        .filter(service => service.isActive)
                        .slice(0, 6)
                        .map((service, index) => (
                          <div key={service.id} className="flex items-center gap-2 p-2 bg-muted/50 rounded">
                  ...
                  <div className="text-xs text-muted-foreground space-y-1">
                    <div>Slides: {sectionData.slidesToShow} | Scroll: {sectionData.slidesToScroll}</div>
                    <div>Autoplay: {sectionData.showAutoplay ? "On" : "Off"} | Speed: {sectionData.autoplaySpeed}ms</div>
                    <div>Navigation: {sectionData.showNavigation ? "On" : "Off"} | Arrows: {sectionData.showArrows ? "On" : "Off"}</div>
                  </div>
                </div>
              </CardContent>
            </Card>
...
const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  return (
    <Card className={`transition-all duration-200 ${!service.isActive ? 'opacity-60' : ''}`}>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="space-y-2">
              {service.bulletPoints.map((point, pointIndex) => (
                <div key={pointIndex} className="flex items-center gap-2">
                  <Input
                    value={point}
                    onChange={(e) => onUpdateBulletPoint(pointIndex, e.target.value)}
